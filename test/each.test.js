...
describe('each', function() {
  ...
  it('should iterate over values and index an array', () => {
    ...
    let i = 0;
    for (const [val, index] of each(arr)) {
      assert.equal(val, `val${i}`);
  ...
  it('should iterate over characters and positions in a string', () => {
    ...
    let i = 0;
    for (const [char, pos] of each(str)) {
  ...
  it('should iterate over keys and values of an object using Symbol.iterator on Object if enabled', () => {
    each.setObjectsIterable();
